# -*- coding: utf-8 -*-
"""
Created on Sun Oct 11 13:15:26 2020
...
from IDDOC.sys_connect import hostname, username, password, database, IDR_host
...
def compare_5_1M_with_db2(infile2, outfile, outile2, model, program_code, year, di_5_1, sk):
 ...
 # Reporting Month: Apr, 2020		Reporting Month Start Date: 04/01/2020		Reporting Month End Date: 04/30/2020			Performance Year: 2020		
    ...
    excel_total_beneficiaries = 0
    excel_total_opt_out = 0
    excel_total_claims_cnt = 0 
    excel_total_claims_amt = 0
    
    for row in excel2.index:
        aco = str(excel2.iloc[row, 0]).strip()
        if not re.match(r'^[FV]\d\d\d$', aco):
            continue
                    
        excel_total_beneficiaries += excel2.iloc[row, 1]
        excel_total_opt_out += excel2.iloc[row, 2]
        excel_total_claims_cnt += excel2.iloc[row, 4]
        excel_total_claims_amt += excel2.iloc[row, 5]
...
### Substance Abuse "Count of SA Claims" validation
    ...
    excel_total_bene_SA = 0
    excel_optin_SA_CLM = 0
    excel_SUPRSD_SA_CLM = 0
    excel_SA_CLM = 0


    for row in excel_sa.index:
        aco = str(excel_sa.iloc[row, 0]).strip()
        if not re.match(r'^[FV]\d\d\d$', aco):
            continue

        excel_total_bene_SA += excel_sa.iloc[row, 5]
        excel_optin_SA_CLM += excel_sa.iloc[row, 7]
        excel_SUPRSD_SA_CLM += excel_sa.iloc[row, 11]
        excel_SA_CLM += excel_sa.iloc[row, 1]
...
#### Retrieve the following from IDR 
#         “Count of SA Claims” 
...
### Total Count of  Excluded Beneficiaries
    ...
    for row in excel_excl.index:        
        aco = str(excel_excl.iloc[row, 0]).strip()
        if not re.match(r'^[FV]\d\d\d$', aco):
            continue
        ...
        for col in excel_excl.columns[1:]:
            ...
            excel_exclusion_values[key] += excel_excl.loc[row, col]
...
#########################################
    sql = f"""
              ...
              FROM  CMS_view_PRVDR_PRD.v1_BENE_SBSTNC_ABUSE_DATA_OPTN
              ;
        """
...
######################################################
    ...
    excel_bene_w_PBP = 0
    for row in excel_pbp.index:
        aco = str(excel_pbp.iloc[row, 0]).strip()
        
        if not re.match(r'^[VF]\d\d\d$', aco):
            continue
        
        excel_bene_w_PBP += excel_pbp.iloc[row, 1]
...
#    with pandas.ExcelWriter(outfile) as writer:
 ...
 #   call object 
    #di_5_1.collect_stats(df, df_sa, df_excl, df_audit, aco_name)
    ...
    report_5_1m_data = Report51MData(header)
